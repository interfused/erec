...
class Ultimate_Ads_Manager_Settings  {
    ...
    public function __construct($plugin_name,$version, $display_name ) {
        ...
        $this->general_settings_key = $cc_uam_config['general_settings_key'];
    ...
    public function register_general_settings() {
        ...
        add_settings_field( 'ga_tracking_id', __('Google Analytics Tracking ID'), array( &$this, 'field_ga_tracking_id' ), $this->general_settings_key, $section_key );
        add_settings_field( 'db_optimization', __('Database Optimization'), array( &$this, 'db_optimization' ), $this->general_settings_key, $section_key );

        //add_settings_field( 'block_timezone', __('Your Timezone'), array( &$this, 'field_block_timezone' ), $this->general_settings_key, $section_key );

    }
    public function section_general_desc() { echo ''; }
    ...
    public function field_ga_tracking_id() {
        $value = isset($this->general_settings['ga_tracking_id']) ? $this->general_settings['ga_tracking_id'] : '';
        ?>
            <label title="GGoogle Analytics Tracking ID">
                <input type="text" name="<?php echo $this->general_settings_key; ?>[ga_tracking_id]"
    ...
    public function db_optimization_OUT(){
        global $cc_uam_config;
        $value = isset($this->general_settings['db_optimization_enabled']) ? $this->general_settings['db_optimization_enabled'] : false;

        ?>
        <label title="Database Optimization">
<!--            --><?php //foreach ($cc_uam_config['forgettable_properties'] as $prop){ ?>
<!--            <label title="--><?php //echo $prop; ?><!--" ></label>-->
<!--                <input type="checkbox" value="--><?php //echo $prop; ?><!--" name="--><?php //echo $this->general_settings_key; ?><!--[dp_optimization][forget][]"/>-->
<!--            --><?php //} ?>
        </label>
        <?php
    }

    public function db_optimization() {
        ?>
        <label title="Block AdBlock">
            <input type="checkbox" name="<?php echo $this->general_settings_key; ?>[block_adblock]"  <?php checked( $this->general_settings['block_adblock'], 1 ); ?>  value="1" />
            <small><?php echo __('Success not guaranteed.'); ?></small>
