      ...
      if (isset($_REQUEST["param"])) {
         ...
         switch (esc_attr($_REQUEST["param"])) {
            case "wizard_clean_up_optimizer":
               if (wp_verify_nonce((isset($_REQUEST["_wp_nonce"]) ? esc_attr($_REQUEST["_wp_nonce"]) : ""), "clean_up_optimizer_check_status")) {
            ...
            case "manual_clean_up_module":
               if (wp_verify_nonce(isset($_REQUEST["_wp_nonce"]) ? esc_attr($_REQUEST["_wp_nonce"]) : "", "wordpress_data_manual_clean_up")) {
                  $types = isset($_REQUEST["data"]) ? json_decode(stripslashes(html_entity_decode($_REQUEST["data"]))) : "";
            ...
            case "manual_clean_up_empty_module":
               if (wp_verify_nonce(isset($_REQUEST["_wp_nonce"]) ? esc_attr($_REQUEST["_wp_nonce"]) : "", "empty_manual_clean_up")) {
            ...
            case "bulk_action_manual_clean_up_module":
               if (wp_verify_nonce(isset($_REQUEST["_wp_nonce"]) ? esc_attr($_REQUEST["_wp_nonce"]) : "", "manual_db_bulk_action")) {
                  $action = isset($_REQUEST["table_action"]) ? esc_attr($_REQUEST["table_action"]) : "";
                  $table_name = isset($_REQUEST["data"]) ? json_decode(stripslashes(html_entity_decode($_REQUEST["data"]))) : "";
            ...
            case "select_action_manual_clean_up_module":
               if (wp_verify_nonce(isset($_REQUEST["_wp_nonce"]) ? esc_attr($_REQUEST["_wp_nonce"]) : "", "manual_db_select_action")) {
                  $action = isset($_REQUEST["perform_action"]) ? esc_attr($_REQUEST["perform_action"]) : "";
                  $table_name = isset($_REQUEST["table_name"]) ? esc_attr($_REQUEST["table_name"]) : "";
                  switch ($action) {
                     case "optimize":
                        $wpdb->query
                            (
            ...
            case "other_settings_module":
               if (wp_verify_nonce(isset($_REQUEST["_wp_nonce"]) ? esc_attr($_REQUEST["_wp_nonce"]) : "", "clean_up_other_settings")) {
                  ...
                  $update_clean_up_type["live_traffic_monitoring"] = isset($other_settings_array["ux_ddl_live_traffic_monitoring"]) ? esc_attr($other_settings_array["ux_ddl_live_traffic_monitoring"]) : "";
                  $update_clean_up_type["visitor_logs_monitoring"] = isset($other_settings_array["ux_ddl_visitor_log_monitoring"]) ? esc_attr($other_settings_array["ux_ddl_visitor_log_monitoring"]) : "";
                  $update_clean_up_type["remove_tables_uninstall"] = isset($other_settings_array["ux_ddl_remove_tables"]) ? esc_attr($other_settings_array["ux_ddl_remove_tables"]) : "";
                  $update_clean_up_type["ip_address_fetching_method"] = isset($other_settings_array["ux_ddl_ip_address_fetching_method"]) ? esc_attr($other_settings_array["ux_ddl_ip_address_fetching_method"]) : "";
                  $update_data = array();
            ...
            case "delete_selected_traffic_module":
               if (wp_verify_nonce(isset($_REQUEST["_wp_nonce"]) ? esc_attr($_REQUEST["_wp_nonce"]) : "", "traffic_delete")) {
            ...
            case "visitor_log_delete_module":
               if (wp_verify_nonce(isset($_REQUEST["_wp_nonce"]) ? esc_attr($_REQUEST["_wp_nonce"]) : "", "visitor_log_delete")) {
                  $confirm_id = isset($_REQUEST["confirm_id"]) ? intval($_REQUEST["confirm_id"]) : 0;
                  $where_meta = array();
                  $where_parent = array();
                  $where_meta["meta_id"] = $confirm_id;
            ...
            case "delete_selected_recent_module":
               if (wp_verify_nonce(isset($_REQUEST["_wp_nonce"]) ? esc_attr($_REQUEST["_wp_nonce"]) : "", "recent_selected_delete")) {
            ...
            case "blocking_options_module":
               if (wp_verify_nonce(isset($_REQUEST["_wp_nonce"]) ? esc_attr($_REQUEST["_wp_nonce"]) : "", "clean_up_block")) {
                  parse_str(isset($_REQUEST["data"]) ? base64_decode($_REQUEST["data"]) : "", $blocking_option_array);
                  $update_type = array();
                  $where = array();

                  $update_type["auto_ip_block"] = isset($blocking_option_array["ux_ddl_auto_ip"]) ? esc_attr($blocking_option_array["ux_ddl_auto_ip"]) : "";
                  ...
                  $update_type["block_for"] = isset($blocking_option_array["ux_ddl_blocked_for"]) ? esc_attr($blocking_option_array["ux_ddl_blocked_for"]) : "";
            ...
            case "manage_ip_address_module":
               if (wp_verify_nonce(isset($_REQUEST["_wp_nonce"]) ? esc_attr($_REQUEST["_wp_nonce"]) : "", "clean_up_manage_ip_address")) {
                  parse_str(isset($_REQUEST["data"]) ? base64_decode($_REQUEST["data"]) : "", $advance_security_data);
                  $ip = isset($_REQUEST["ip_address"]) ? ip2long(json_decode(stripslashes($_REQUEST["ip_address"]))) : "";
                  ...
                  if (isset($ip_address_ranges_data) && count($ip_address_ranges_data) > 0) {
                     foreach ($ip_address_ranges_data as $data) {
                        ...
                        $ip_range_unserialized_data_match = isset($ip_range_unserialized_data["ip_range"]) ? esc_attr($ip_range_unserialized_data["ip_range"]) : "";
                  ...
                  if ($ip_exists == true) {
                     ...
                     $insert_manage_ip_address["blocked_for"] = isset($advance_security_data["ux_ddl_ip_blocked_for"]) ? esc_attr($advance_security_data["ux_ddl_ip_blocked_for"]) : "";
                     $insert_manage_ip_address["location"] = isset($location) ? esc_html($location) : "";
                     $insert_manage_ip_address["comments"] = isset($advance_security_data["ux_txtarea_ip_comments"]) ? esc_attr($advance_security_data["ux_txtarea_ip_comments"]) : "";
                     ...
                     $time_interval = isset($advance_security_data["ux_ddl_ip_blocked_for"]) ? esc_attr($advance_security_data["ux_ddl_ip_blocked_for"]) : "";
            ...
            case "delete_ip_address_module":
               if (wp_verify_nonce(isset($_REQUEST["_wp_nonce"]) ? esc_attr($_REQUEST["_wp_nonce"]) : "", "clean_up_manage_ip_address_delete")) {
            ...
            case "delete_ip_range_module":
               if (wp_verify_nonce(isset($_REQUEST["_wp_nonce"]) ? esc_attr($_REQUEST["_wp_nonce"]) : "", "clean_up_manage_ip_ranges_delete")) {
            ...
            case "manage_ip_ranges_module":
               if (wp_verify_nonce(isset($_REQUEST["_wp_nonce"]) ? esc_attr($_REQUEST["_wp_nonce"]) : "", "clean_up_manage_ip_ranges")) {
                  parse_str(isset($_REQUEST["data"]) ? base64_decode($_REQUEST["data"]) : "", $ip_range_data);
                  $start_ip_range = isset($_REQUEST["start_range"]) ? ip2long(json_decode(stripslashes($_REQUEST["start_range"]))) : "";
                  $end_ip_range = isset($_REQUEST["end_range"]) ? ip2long(json_decode(stripslashes($_REQUEST["end_range"]))) : "";
                  ...
                  if (isset($ip_address_range_data) && count($ip_address_range_data) > 0) {
                     foreach ($ip_address_range_data as $data) {
                        $ip_range_unserialized_data = maybe_unserialize($data->meta_value);
                        $ip_range_unserialized_match_data = isset($ip_range_unserialized_data["ip_range"]) ? esc_attr($ip_range_unserialized_data["ip_range"]) : "";
                  ...
                  if ($ip_exists == false) {
                     ...
                     $insert_manage_ip_range["blocked_for"] = isset($ip_range_data["ux_ddl_range_blocked"]) ? esc_attr($ip_range_data["ux_ddl_range_blocked"]) : "";
                     ...
                     $time_interval = isset($ip_range_data["ux_ddl_range_blocked"]) ? esc_attr($ip_range_data["ux_ddl_range_blocked"]) : "";
                     if ($time_interval != "permanently") {
            ...
            case "change_email_template_module":
               if (wp_verify_nonce(isset($_REQUEST["_wp_nonce"]) ? esc_attr($_REQUEST["_wp_nonce"]) : "", "email_template_data")) {
