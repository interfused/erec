...
if( $key ) $field[ 'meta_key' ] = $key;
	...
	foreach ( $field[ 'options' ] as $key => $value ) :
		...
		if ( $replace_default > 0 && $replace_disabled < 1 ) $field[ 'default' ] = $key;
			...
			?>
			<label>
			<input type="radio" class="<?php echo esc_attr( implode( ' ', $classes ) ); ?>" name="<?php echo esc_attr( isset( $field[ 'name' ] ) ? $field[ 'name' ] : $field['meta_key'] ); ?>" id="<?php echo $field['meta_key'] . '-' . esc_attr( $key ); ?>" value="<?php echo esc_attr( $key ); ?>" <?php if ( isset( $field[ 'value' ] ) || isset( $field[ 'default' ] ) ) checked( isset( $field[ 'value' ] ) ? $field[ 'value' ] : $field[ 'default' ], $key, TRUE ); ?> 
				<?php echo $disabled_option; ?>/><span><?php echo esc_html( $value ); ?></span>
			</label>
			<?php
	endforeach;
