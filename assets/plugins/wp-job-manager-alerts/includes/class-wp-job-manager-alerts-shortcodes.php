			...
			try {
				switch ( $this->action ) {
					...
					case 'cancel_alert' :
						...
						$update_alert = array();
						$update_alert['ID'] = $alert_id;
						$update_alert['post_status'] = $alert->post_status == 'publish' ? 'draft' : 'draft';
						wp_update_post( $update_alert );

						// Message
						$this->alert_message = '<div class="job-manager-message">' . sprintf( __( '%s has been %s', 'wp-job-manager-alerts' ), $alert->post_title, $alert->post_status == 'draft' ? __( 'Disabled', 'wp-job-manager-alerts' ) : __( 'Disabled', 'wp-job-manager-alerts' ) ) . '</div>';
					break;
					case 'toggle_status' :
						$alert_id = absint( $_REQUEST['alert_id'] );
						$alert    = get_post( $alert_id );
