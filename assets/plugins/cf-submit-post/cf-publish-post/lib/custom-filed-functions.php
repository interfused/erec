...
/**
 * cf_publish_post_custom_field_List_Table class that will display our custom table
 * records in nice table
 */
class cf_publish_post_custom_field_List_Table extends WP_List_Table
{
    /**
    ...
    function column_customfield($item)
    {
        ...
        $actions = array(
            'edit' => sprintf('<a href="?page=cf-publish-post&tab=cf-publish-post-custom-field&id=%s">%s</a>', $item['id'], __('Edit', 'custom_field')),
            'delete' => sprintf('<a href="?page=%s&tab=cf-publish-post-custom-field&action=delete&id=%s">%s</a>', $_REQUEST['page'], $item['id'], __('Delete', 'custom_field')),
        );
...
function cf_publish_post_custom_field_persons_form_meta_box_handler($item)
{
    ?>
...
<table cellspacing="2" cellpadding="5" style="width: 100%;" class="form-table">
  <tbody>
    <tr valign="top">
      <td scope="row" class="label"><label for="field">Custom Field</label></td>
      <td><input size="25" style="" id="customfield" value="<?php echo esc_attr($item['customfield'])?>" name="customfield" type="text" placeholder="<?php _e('Field Name', 'custom_field')?>">
        <span>Enter custom field name where value store Ex:- my_custom_field</span></td>
    </tr>
    <tr valign="top">
      <td scope="row" class="label"><label for="label">Contact Field</label></td>
      <td><input size="25" style="" id="label" value="<?php echo esc_attr($item['customlabel'])?>" name="customlabel" type="text" placeholder="<?php _e('Label Name', 'custom_field')?>">
        <span>Enter Contact form 7 Field Name from where you want to get value Ex:- your-name</span></td>
    </tr>
  </tbody>
</table>
<?php
 ...
 * @return bool|string
 */
function cf_publish_post_custom_field_validate_person($item)
{
    $messages = array();
 ...
 */
function cf_publish_post_custom_field_languages()
{
    load_plugin_textdomain('custom_field', false, dirname(plugin_basename(__FILE__)));
}

add_action('init', 'cf_publish_post_custom_field_languages');
