...
?>
    ...
    $table = new cf_publish_post_custom_field_List_Table();
    $table->prepare_items();
    ...
    ?>
          ...
          <h2><a class="add-new-h2"
                                href="<?php echo get_admin_url(get_current_blog_id(), 'admin.php?page=cf-publish-post&tab=cf-publish-post-custom-field');?>">
    ...
    if (wp_verify_nonce($_REQUEST['nonce'], basename(__FILE__))) {
        ...
        $item_valid = cf_publish_post_custom_field_validate_person($item);
        if ($item_valid === true) {
            if ($item['id'] == 0) {
    ...
    add_meta_box('persons_form_meta_box', 'Custom Field', 'cf_publish_post_custom_field_persons_form_meta_box_handler', 'person', 'normal', 'default');
