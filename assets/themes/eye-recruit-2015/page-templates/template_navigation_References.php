    ...
    <div id="content" class="container" role="main">
      ...
      <section class="navigations">
        <div class="section_title">
          <h3>Reference <?php echo $breadText; ?></h3>
          <span><strong>Recruit ID</strong> : <?php echo $userID; ?></span>
        </div>
        <?php
        global $wpdb;
        $tablename = $wpdb->prefix.'er_references';
        $select = $wpdb->get_results("SELECT * FROM $tablename WHERE user_id = '".$userID."' AND rcvr_pref_contact_method IS NOT NULL  ORDER BY id DESC ");
        $selectPending = $wpdb->get_results("SELECT * FROM $tablename WHERE user_id = '".$userID."' AND rcvr_pref_contact_method IS NULL ORDER BY id DESC ");
        $countref = count($select); 
        ?>
        <ol class="breadcrumb paddedLR">
          <li><a href="<?php echo $breadcrumbUrl; ?>">Home</a></li>
          <li class="active">Reference <?php echo $breadText; ?></li>
        </ol>
        <div class="row indent">
          <div class="col-md-9">
            ...
            if(count($selectPending) > 0 && $seekerPreviewAsEmployer != true ){
              ...
              <?php if($view == 'allow'){ ?>
              <h3>Pending References</h3>
              <div class="search_bar">
                <p><?php echo $cand_name." ".$verb; ?> <span id="counttotalpendingref" count="<?php echo count($selectPending); ?>"><?php echo count($selectPending); ?></span> pending reference(s)</p>
              </div>
              ...
              <?php
                ...
                <h3>Completed References</h3>

                <?php if($view == 'allow'){ ?>
                <div class="search_bar">
                  <p><?php echo $cand_name." ".$verb; ?> <span id="counttotalref" count="<?php echo $countref; ?>"><?php echo $countref; ?></span> completed reference(s)</p>
                </div>
                <?php } ?>
                    ...
                    <div class="col-md-12">
                      ...
                      <div class="" id="inv_frn_n_coll">
                        <?php foreach ($select as $value) {
                          ...
                          ?>
                          <div id="refralno<?php echo $value->id; ?>" class="col-md-6">
                            <article>
