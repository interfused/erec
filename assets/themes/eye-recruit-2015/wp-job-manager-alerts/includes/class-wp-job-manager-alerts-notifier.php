...
/* END HACK */
			...
			remove_filter( 'wp_mail_from', array( $this, 'set_content_type' ) );
		}
	...
	public function format_email( $alert, $user, $jobs ) {
		...
		//$action_url = remove_query_arg( 'updated', add_query_arg( array( 'action' => 'toggle_status', 'alert_id' => $alert->ID ) ) );
		$action_url = site_url().'/job-seekers/job-alerts/?action=cancel_alert&alert_id='.$alert->ID;
	...
	/** SET HTML  EMAIL . INTERFUSED HACK
	*/
