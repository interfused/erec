...
/*..............AjAx forward Job seeker Resume..................*/
		        ...
		        if ( $_FILES['files']['size'][$f] > $max_file_size ) {
			        ...
			        if( move_uploaded_file( $_FILES["files"]["tmp_name"][$f], $path.$basename ) ) {
						...
						if ( isset($thank_job_apply['thank_job_apply_subject']) ) {
							...
							$thank_subject = 'Thank You for Apply';
						}
...
/*..............AjAx demo seeker  send Resume..................*/
add_action('wp_ajax_applyforjobwithoutlogin_demo', 'applyforjobwithoutlogin_demo');
add_action('wp_ajax_nopriv_applyforjobwithoutlogin_demo', 'applyforjobwithoutlogin_demo');
function applyforjobwithoutlogin_demo(){
	global $wpdb;
    ...
    if( $_SERVER['REQUEST_METHOD'] == "POST" ){
    	...
    	$from_name = $_POST['uname'];
    	//$postid = 4728;
    	$author_name = $from_name.' '.$lastname;
    /*	$postdata = get_post($postid);
    	$author_id = $postdata->post_author;
    	$authordata = get_userdata($author_id);
    	$author_name = $authordata->first_name.' '.$authordata->last_name;
    	...
    	foreach ( $_FILES['files']['name'] as $f => $name ) {
		    ...
		    if ( $_FILES['files']['error'][$f] == 0 ) {
		        if ( $_FILES['files']['size'][$f] > $max_file_size ) {
		            ...
		            $upload_message[] = 'File is not a valid format. Allow only pdf, jpg, jpeg and png format.';
		            continue;
		        } 
		        else{
					$i = 1;
			        ...
			        if( move_uploaded_file( $_FILES["files"]["tmp_name"][$f], $path.$basename ) ) {
						...
						$Replaceto = array('[site_url]', '[author_name]', '[job_title]', '[from_name]', '[from_email]', '[job_url]');
						$Replaceby = array(site_url(), $author_name, '', $from_name, '', '');

						if ( isset($job_apply['job_apply_template']) ) {
							$message = str_replace($Replaceto, $Replaceby, $job_apply['job_apply_template']);
						}
						else{
							$message = 'A candidate '.$from_name.' has submitted there application';
						}
						...
						if( wp_mail('chris.bauer@eyerecruit.com', $subject, $message, $headers, $mail_attachment) ){
							//wp_mail($from_email, $thank_subject, $thank_message, $headers);
						}
