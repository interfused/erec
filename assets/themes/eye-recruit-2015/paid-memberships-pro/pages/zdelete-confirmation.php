...
<?php 
	...
	if(empty($current_user->membership_level))
		$confirmation_message = "<h5>" . __('Your payment has been submitted. Your membership will be activated shortly.', 'pmpro') . "</h5>";
	else
		$confirmation_message = "<h5>" . sprintf(__('Thank you for your membership to %s. Your %s membership is now active.', 'pmpro'), get_bloginfo("name"), $current_user->membership_level->name) . "</h5>";		
...
<?php if(!empty($pmpro_invoice) && !empty($pmpro_invoice->id)) { ?>		
	...
	<?php
		...
		$confirmation_message .= "<p class='text-center'>" . sprintf(__('Below are details about your membership account and a receipt for your initial membership invoice. A welcome email with a copy of your initial membership invoice has been sent to %s.', 'pmpro'), $pmpro_invoice->user->user_email) . "</p>";
	...
	<div class="sidebar_title cont_title">
		<h4><?php printf(__('Invoice #%s on %s', 'pmpro'), $pmpro_invoice->code, date_i18n(get_option('date_format'), $pmpro_invoice->timestamp));?></h4>
	...
	<ul class="card_billing_info">
		...
		<li><span><?php _e('Account', 'pmpro');?>:</span> <?php echo $current_user->display_name?> (<?php echo $current_user->user_email?>)</li>
		<li><span><?php _e('Membership Level', 'pmpro');?>:</span> <?php echo $current_user->membership_level->name?></li>
		<?php if($current_user->membership_level->enddate) { ?>
			<li><span><?php _e('Membership Expires', 'pmpro');?>:</span> <?php echo date_i18n(get_option('date_format'), $current_user->membership_level->enddate)?></li>
		<?php } ?>
		<?php if($pmpro_invoice->getDiscountCode()) { ?>
			<li><span><?php _e('Discount Code', 'pmpro');?>:</span> <?php echo $pmpro_invoice->discount_code->code?></li>
		<?php } ?>
		<?php do_action("pmpro_invoice_bullets_bottom", $pmpro_invoice); ?>
	</ul>
	<div class="table-responsive">
	<table id="pmpro_confirmation_table" class="table table-bordered pmpro_invoice" width="100%" cellpadding="0" cellspacing="0" border="0">
		<thead>
			<tr>
