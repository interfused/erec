...
// prepopulate
if(!$_POST){
  $bfirstname = $bfirstname ? $bfirstname : $current_user->first_name;
  $blastname = $blastname ? $blastname : $current_user->last_name;
  $bphone = $bphone ? $bphone : get_user_meta($current_user->ID, 'cell_phone', true);
}
    ...
    <form id="pmpro_form" class="pmpro_form" action="<?php if(!empty($_REQUEST['review'])) echo pmpro_url("checkout", "?level=" . $pmpro_level->id); ?>" method="post">
      ...
      <table id="pmpro_pricing_fields" class="pmpro_checkout" width="100%" cellpadding="0" cellspacing="0" border="0">
        ...
        <tbody>
            ...
            <td>
              <div class="membership_about">
                <div class="sidebar_title cont_title">
                  <h4>
                    <?php printf(__('<strong>%s</strong> membership', 'paid-memberships-pro' ), $pmpro_level->name);?>
                  </h4>
                </div>
          ...
          <h4 class="text-primary">This order form is provided in a secure environment and to help protect against fraud, your current IP address <?php echo $_SERVER['REMOTE_ADDR']; ?> is being logged.</h4>
        ...
        <td>
          ...
          else
          {
            ...
            <div>
              ...
              {
                ?>
                <div id="country-wrapper" class="col-md-6">
                <div class="form-group">
...
<div class="col-md-6">
  <div class="form-group pmpro_payment-expiration">
...
if($pmpro_show_cvv) { ?>
<div class="col-md-6">
  <div class="form-group pmpro_payment-cvv">
    <label for="CVV"><?php _e('CVV', 'paid-memberships-pro' );?></label>
    <input class="input" id="CVV" name="CVV" type="text" size="4" value="<?php if(!empty($_REQUEST['CVV'])) { echo esc_attr($_REQUEST['CVV']); }?>" class="<?php echo pmpro_getClassForField("CVV");?>" />  <p class="text-right fineprint">(<a href="javascript:void(0);" onclick="javascript:window.open('<?php echo pmpro_https_filter(PMPRO_URL)?>/pages/popup-cvv.html','cvv','toolbar=no, location=no, directories=no, status=no, menubar=no, scrollbars=yes, resizable=yes, width=600, height=475');"><?php _e("what's this?", 'paid-memberships-pro' );?></a>)</p>
  </div>
</div>
<?php } ?>
...
<div class="sidebar_title cont_title">
  ...
  if($pmpro_checkout_default_submit_button)
  {
    ...
    <span id="pmpro_submit_span">
      ...
      <input type="submit" class="pmpro_btn pmpro_btn-submit-checkout btn btn-primary" value="<?php if($pmpro_requirebilling) { _e('Submit and Check Out', 'paid-memberships-pro' ); } else { _e('Submit and Confirm', 'paid-memberships-pro' );}?> &raquo;" />
    </span>
    <?php
  }
  ?>
...
<!--
  ...
  jQuery('.pmpro_required').closest('div').find('label').append('<span class="pmpro_asterisk"> <abbr title="Required Field">*</abbr></span>');
