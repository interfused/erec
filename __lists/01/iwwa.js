...
var scrapeURL = 'http://iwwa.euro-detectives.org/world1.html';
var formData = {
...
$('.text-bold').each(function(index, el) {
  contactID++;
  let contact = {};
  contact.list_source = 'INVESTIGATIONS WORLDWIDE ASSOCIATION';
...
/*
  if(lines.length > 1){
    contact.fullname = lines[1];
  }
  */

  //contact.fullname = trim( $(this).text() );
  //contact.fullname = detailTextNodes[0].nodeValue;
...
/*
  ...
  */
  contact.email = trim( $(this).find('a').eq(0).text() );
  contact.website = trim( $(this).find('a').eq(1).text() );
  contact.misc_data = trim( $(this).text() );
...
//    console.dir(detailTextNodes);
    ...
    console.log('web: '+contact.website);
    console.log('misc: '+contact.misc_data);
/*
